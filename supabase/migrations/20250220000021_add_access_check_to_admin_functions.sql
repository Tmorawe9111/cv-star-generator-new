-- Migration: Add access code check to admin functions
-- Purpose: Require admin access code for admin_assign_plan and admin_add_tokens

-- ============================================
-- 1. Update admin_assign_plan to check access
-- ============================================
...
-- ============================================
-- 2. Update admin_add_tokens to check access
-- ============================================
...
BEGIN
  ...
  IF NOT EXISTS (
    ...
    RAISE EXCEPTION 'Nur Admins können Tokens verwalten';
  END IF;

  -- Check if admin has access to this company (via access code)
  IF NOT public.admin_has_company_access(v_admin_id, p_company_id) THEN
    RAISE EXCEPTION 'Sie benötigen einen Freigabe-Code, um Tokens für dieses Unternehmen zu ändern. Bitte geben Sie den Code im Unternehmensprofil ein.';
  END IF;
...
GRANT EXECUTE ON FUNCTION public.admin_add_tokens TO authenticated;
