-- Retry migration without IF NOT EXISTS on policies and with idempotent trigger creation
...
-- Recreate trigger safely
drop trigger if exists trg_pages_log_revision on public.pages;
...
create policy "Editors can manage blog images"
