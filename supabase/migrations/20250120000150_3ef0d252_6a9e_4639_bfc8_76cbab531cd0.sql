-- Token/Seat/Plan system schema and RPCs
...
-- 2) Company subscription state (single row per company)
...
-- 3) Token pricing tiers (volume discounts)
...
alter table public.company_subscriptions
  add constraint if not exists company_subscriptions_non_negative check (seats >= 0 and token_balance >= 0);
...
create policy if not exists "Company members can read subscription"
  on public.company_subscriptions for select
  using (public.has_company_access(company_id));

create policy if not exists "Company members can read token ledger"
  on public.token_ledger for select
  using (public.has_company_access(company_id));

create policy if not exists "Company members can read seat ledger"
  on public.seat_ledger for select
  using (public.has_company_access(company_id));

create policy if not exists "Company members can read plan changes"
  on public.plan_changes for select
  using (public.has_company_access(company_id));

-- Block direct writes from clients (mutations via RPCs only)
...
-- Pricing tables readable to everyone (or tighten if desired)
alter table public.plans enable row level security;
create policy if not exists "Plans readable by everyone"
  on public.plans for select using (true);

alter table public.token_pricing_tiers enable row level security;
create policy if not exists "Token pricing readable by everyone"
  on public.token_pricing_tiers for select using (active);

alter table public.seat_pricing_tiers enable row level security;
create policy if not exists "Seat pricing readable by everyone"
  on public.seat_pricing_tiers for select using (active);

-- RPCs
-- Helper: token unit price
