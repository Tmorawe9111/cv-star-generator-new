-- Fix unlock_candidate_profile JSONB parsing issue
-- The error "cannot extract elements from a scalar" means we're trying to parse a string as JSONB
...
BEGIN
  ...
  IF p_linked_job_ids IS NOT NULL THEN
    BEGIN
      -- Try to extract as JSONB array
      SELECT ARRAY(
        SELECT jsonb_array_elements_text(p_linked_job_ids)::uuid
      ) INTO v_job_ids;
    EXCEPTION WHEN OTHERS THEN
      -- If that fails, it might be a string - convert to array
