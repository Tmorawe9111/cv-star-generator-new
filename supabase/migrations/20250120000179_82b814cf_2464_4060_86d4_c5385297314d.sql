...
-- Prevent reverse duplicates (A,B) vs (B,A): only one row per pair
DO $$ BEGIN
  CREATE UNIQUE INDEX uniq_connections_pair
...
-- RLS: insert own outgoing requests
...
-- RLS: update by participants (restricted transitions handled by business logic/UI)
CREATE POLICY IF NOT EXISTS "Participants can update connection status"
ON public.connections
FOR UPDATE
USING (requester_id = auth.uid() OR addressee_id = auth.uid());

-- Optional: allow requester to delete pending (cancel)
CREATE POLICY IF NOT EXISTS "Requester can delete pending requests"
ON public.connections
FOR DELETE
USING (requester_id = auth.uid() AND status = 'pending');

-- Trigger: set responded_at when leaving pending
...
CREATE POLICY IF NOT EXISTS "Blocker manages their blocks"
ON public.blocks
FOR ALL
...
CREATE POLICY IF NOT EXISTS "Participants can view conversations"
ON public.conversations
FOR SELECT
USING (a_id = auth.uid() OR b_id = auth.uid());

-- will restrict creation with is_connected below
CREATE POLICY IF NOT EXISTS "Participants can create conversations if connected"
ON public.conversations
FOR INSERT
...
AS $$
  SELECT
    -- Both directions considered same pair; accepted connection and not blocked
    EXISTS (
...
-- 6) Trigger to auto-create conversation when a connection becomes accepted
...
BEGIN
  IF TG_OP = 'UPDATE' AND NEW.status = 'accepted' AND OLD.status IS DISTINCT FROM 'accepted' THEN
    ...
    -- Upsert conversation for this pair
