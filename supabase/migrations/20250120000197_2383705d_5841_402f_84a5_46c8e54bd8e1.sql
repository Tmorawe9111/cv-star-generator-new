...
-- Optional: restrict selects to owner
create policy if not exists "Users can view their own suggestion history"
  on public.suggestions_history
  for select
  to authenticated
  using (auth.uid() = profile_id);

-- Allow inserts/updates via security definer function only (no direct writes)
create policy if not exists "No direct writes to suggestions_history"
  on public.suggestions_history
  for all
  to authenticated
  using (false)
