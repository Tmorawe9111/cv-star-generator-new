-- Fix unlock_candidate_profile to set status to FREIGESCHALTET when unlocking
-- This ensures newly unlocked profiles are counted correctly in dashboard metrics
...
BEGIN
  ...
  IF p_linked_job_ids IS NOT NULL THEN
    BEGIN
      SELECT ARRAY(
        SELECT jsonb_array_elements_text(p_linked_job_ids)::uuid
      ) INTO v_job_ids;
  ...
  IF FOUND THEN
    -- Update existing record - ensure status is set to FREIGESCHALTET if not already set
    ...
    SET 
      ...
      unlocked_at = COALESCE(unlocked_at, now()), -- Set if not already set
      status = COALESCE(status, 'FREIGESCHALTET'::candidate_status), -- Set to FREIGESCHALTET if NULL
      updated_at = now()
    WHERE company_id = p_company_id 
      AND candidate_id = v_profile_id;
  ELSE
    -- Insert new record - explicitly set status to FREIGESCHALTET
