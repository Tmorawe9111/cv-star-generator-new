...
-- 4. Delete orphaned applications without user_id (since they can't be secured)
DELETE FROM public.applications WHERE user_id IS NULL;

-- 5. Now add the NOT NULL constraint
ALTER TABLE public.applications ALTER COLUMN user_id SET NOT NULL;

-- 6. Create secure policies for applications table
...
-- 7. Fix function security - add search_path to existing functions
